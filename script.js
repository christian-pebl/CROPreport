class CSVManager {
    constructor() {
        ...
        // Multi-file support        this.loadedFiles = new Map();        this.activeFilesLength = new Set();        this.activeFilesWidth = new Set();        this.colorPalette = ["#3B82F6", "#EF4444", "#10B981", "#F59E0B", "#8B5CF6"];        this.maxFiles = 5;
...
class NavigationManager {
    ...
    initializeComparisonControls() {
        ...
        // Width distribution controls
        const widthSelect1 = document.getElementById('widthSelect1');
        const sitesSelectWidth1 = document.getElementById('sitesSelectWidth1');
        const generateSiteComparisonWidthBtn = document.getElementById('generateSiteComparisonWidthBtn');


        // Add event listeners for width distribution buttons
        const updateWidthSiteComparisonButton = () => {
            const siteSelected = sitesSelectWidth1?.value;
            const varsSelected = Array.from(widthSelect1?.selectedOptions || []).map(option => option.value);
            if (generateSiteComparisonWidthBtn) {
                generateSiteComparisonWidthBtn.disabled = !siteSelected || varsSelected.length < 1;
            }
        };


        // Add event listeners for width controls
        if (widthSelect1) widthSelect1.addEventListener('change', updateWidthSiteComparisonButton);
        if (sitesSelectWidth1) sitesSelectWidth1.addEventListener('change', updateWidthSiteComparisonButton);
    ...
    updateDropdowns(pageName = 'plot') {
        ...
        // Update width distribution dropdowns
        this.updateWidthDropdowns(hr24Files, this.stdFiles, pageName);
    ...
    updateWidthDropdowns(hr24Files, stdFiles, pageName = 'plot') {
        console.log("🎛️ DEBUG: Starting width distribution dropdown update process");

        // Look for the specific files that should contain width data (like Indiv files)
        const widthDataFiles = [];

        // Check if availableFiles contains the expected files
        if (this.availableFiles && this.availableFiles.length > 0) {
            // Look for Indiv files (which should contain width data)
            const individFiles = this.availableFiles.filter(file =>
                file.name.includes('Indiv') || file.name.includes('_indiv')
            );
            widthDataFiles.push(...individFiles);

            // Also check for Summary files as backup
            const summaryFiles = this.availableFiles.filter(file =>
                file.name.includes('Summary') || file.name.includes('_summary')
            );
            widthDataFiles.push(...summaryFiles);
        }

        console.log("📁 DEBUG: Found width data files:", widthDataFiles.map(f => f.name));
        const idPrefix = "";

        // Update sites dropdown for width site comparison (width data files) - single select
        const sitesSelectWidth1 = document.getElementById(idPrefix + 'sitesSelectWidth1');
        if (sitesSelectWidth1) {
            console.log("🎯 DEBUG: Found sitesSelectWidth1 dropdown, populating with width data files");
            sitesSelectWidth1.innerHTML = '<option value="">Select a CSV file...</option>';
            widthDataFiles.forEach(file => {
                const option = document.createElement('option');
                option.value = file.name;
                option.textContent = file.name;
                sitesSelectWidth1.appendChild(option);
            });
        }

        // Update site dropdown for width variable comparison (width data files)
        const siteSelectWidth2 = document.getElementById(idPrefix + 'siteSelectWidth2');
        if (siteSelectWidth2) {
            console.log("🎯 DEBUG: Found siteSelectWidth2 dropdown, populating with width data files");
            siteSelectWidth2.innerHTML = '<option value="">Select a CSV file...</option>';
            widthDataFiles.forEach(file => {
                const option = document.createElement('option');
                option.value = file.name;
                option.textContent = file.name;
                siteSelectWidth2.appendChild(option);
            });
        }
    }
...
formatTimePointsAsDateLabels(sortedHours, sampleSiteData, formatType = "date") {        // For 24hr file format - use time format when requested (check this FIRST)        if (formatType === "time") {            return sortedHours.map((hour) => {                const hourNum = hour.includes("_") ? parseInt(hour.split("_")[1], 10) : parseInt(hour, 10);                const hours = String(hourNum).padStart(2, "0");                return `${hours}:00`;            });        }        // Check if we're dealing with std file time identifiers (format: "YYYY-MM-DD_HH")        if (sortedHours.length > 0 && sortedHours[0].includes("_")) {            // This is std file format with date_hour identifiers            return sortedHours.map(timeIdentifier => {                const [dateStr, hourStr] = timeIdentifier.split("_");                const date = new Date(dateStr + "T00:00:00Z");                const day = String(date.getDate()).padStart(2, "0");                const month = String(date.getMonth() + 1).padStart(2, "0");                const year = String(date.getFullYear()).slice(-2);                return `${day}/${month}/${year}`;            });        }        // For 24hr file format - use time format when requested
    ...
    // Width distribution methods
    async generateWidthDistributionFromFirstCard(site, widthVars) {
        console.log('=== GENERATE WIDTH DISTRIBUTION FROM FIRST CARD START ===');
        console.log('Site:', site);
        ...
        outputDiv.innerHTML = `
            <div style="background: #f0f9ff; border: 1px solid #0ea5e9; border-radius: 6px; padding: 15px; text-align: center;">
                <h4 style="color: #0369a1; margin-bottom: 8px;">🔄 Generating Width Distribution Plot...</h4>
                <p>Loading ${site} data for variables: ${widthVars.join(', ')}</p>
            </div>
        `;

        try {
            console.log('Starting to load file for width distribution...');
            // Load the CSV file using the same approach as Blade Count
            const rawData = await this.loadWidthData(site);

            console.log(`Loaded ${rawData.length} rows of data for width distribution`);
            ...
            console.error('Error in width distribution chart creation:', error);
            outputDiv.innerHTML = `
                <div style="background: #fef2f2; border: 1px solid #f87171; border-radius: 6px; padding: 15px;">
                    <h4 style="color: #dc2626; margin-bottom: 8px;">❌ Error</h4>
                    <p><strong>Plot creation failed:</strong> ${error.message}</p>
    ...
    async generateWidthSiteComparison(widthVar, sites) {
        console.log('=== GENERATE WIDTH SITE COMPARISON START ===');
        console.log('Width Variable:', widthVar);
        console.log('Sites:', sites);

        // This method would be for comparing the same variable across multiple sites
        // For now, redirect to the single-site comparison since we typically work with one file at a time
        const outputDiv = document.getElementById('variableComparisonWidthOutput');
        if (!outputDiv) {
            console.error('Width variable comparison output div not found');
            return;
        }
    ...
    async generateWidthVariableComparison(site, widthVars) {
        console.log('=== GENERATE WIDTH VARIABLE COMPARISON START ===');
        console.log('Site:', site);
        console.log('Width Variables:', widthVars);
    ...
    renderWidthDistributionChart(rawData, widthVars, outputDiv, filename = 'unknown') {
        ...
        try {
            ...
            if (!widthVars || !Array.isArray(widthVars) || widthVars.length === 0) {
                throw new Error('No width variables provided');
            }
            ...
            // Map AV variable to individual column name
            ...
            if (individualColumn) {
                ...
                try {
                    ...
                    console.log('⚠️ Individual data extraction failed, falling back to averages:', error.message);
                }
            }
            ...
            console.log(`📈 Chart data prepared: ${aggregatedData.length} stations, type: ${chartType}`);

            // Create toggle buttons for chart/table view
            ...
            // Add chart container to output div
            outputDiv.innerHTML = '';
            ...
            tableBtn.addEventListener('click', () => {
                ...
                if (!table) {
                    table = this.createWidthDistributionTable(aggregatedData, selectedVariable);
            ...
            console.error('❌ Error in renderWidthDistributionChart:', error);

            outputDiv.innerHTML = `
                <div style="background: #fef2f2; border: 1px solid #f87171; border-radius: 6px; padding: 15px;">
                    <h4 style="color: #dc2626; margin-bottom: 8px;">❌ Rendering Error</h4>
                    <p>Failed to create width distribution chart</p>
    ...
    createWidthDistributionTable(data, selectedVariable) {
        console.log('📋 Creating width distribution table');
        ...
        const headers = ['Station', 'Count', 'Average'];

        // Add box plot specific headers if applicable
        const hasBoxPlots = data.some(d => d.hasBoxPlot && d.statistics);
        if (hasBoxPlots) {
            headers.push('Min', 'Q1', 'Median', 'Q3', 'Max', 'Outliers');
        }
        ...
        sortedData.forEach(item => {
            ...
            avgCell.textContent = item.average ? item.average.toFixed(2) : '-';
            avgCell.style.cssText = `
                border: 1px solid #e5e7eb;
                padding: 8px;
                text-align: center;
            `;
            ...
            if (hasBoxPlots) {
                ...
                [
                    ...
                    stats?.q3?.toFixed(2) || '-',
                    stats?.max?.toFixed(2) || '-',
                    stats?.outliers?.length > 0 ? stats.outliers.map(o => o.toFixed(2)).join(', ') : 'None'
                ].forEach(value => {
        ...
        console.log('✅ Width distribution table created');
        return table;
    }

    // Filter station measurements by subset (for box plots)
    filterStationMeasurementsBySubset(stationMeasurements, selectedVariable) {
        ...
        console.log(`📊 Target subset: ${targetSubset}`);

        const filteredStationMeasurements = {};

        Object.entries(stationMeasurements).forEach(([station, measurements]) => {
            if (targetSubset === 'all') {
    ...
    // Calculate quantiles for box plot statistics
    calculateQuantile(values, percentile) {
    ...
    // Data aggregation method for simple charts (fallback when box plots aren't available)
    aggregateDataByStationForChart(rawData, selectedVariable) {
        console.log('📊 === AGGREGATING DATA BY STATION FOR SIMPLE CHART ===');
        console.log('Raw data rows:', rawData.length);
        ...
        console.log(`✅ Aggregated data for ${result.length} stations`);
        return result;
    }
